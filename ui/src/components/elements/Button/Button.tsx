...
const variants = cva(
	'inline-block text-white uppercase transition-all duration-150 border border-solid font-bold rounded',
	{
		variants: {
			color: {
				facebook:
			...
			size: {
				...
				regular: ['text-sm', 'px-6', 'py-2'],
				lg: ['text-sm', 'px-6', 'py-3'],
			},
			outline: {
				true: 'outline-none focus:outline-none',
				false: '',
			},
			fullWidth: {
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
	({ className, children, ...props }, ref) => {
		return (
			<button ref={ref} className={cn(variants(props), className)} {...props}>
				{children}
			</button>
		)
	}
